...
def crossover_search(fname, max_iter, pop_size):
	...
	for i in range(max_iter):
		# copy the top 33% of the population to the next generation, and for the rest randomly 
		# cross-breed pairs
		top_half = [p[1] for p in curr_gen[:int(pop_size/3)]]
		next_gen = top_half[:]
		fit_score = [ 1/p[0] for p in curr_gen[:int(pop_size/3)]]
		stand_fit(fit_score)
		while len(next_gen) < pop_size:
			# s = random.choice(top_half)
			# t = random.choice(top_half)
			c1 = pick_one(fit_score)
			c2 = pick_one(fit_score)
			while (c1==c2):
				c2 = pick_one(fit_score)
			s = top_half[c1]
			t = top_half[c2]
			...
			first, second=OX(s, t)
			mutation_section_shuffle(first, 0.2)
			next_gen.append(first)
			if len(next_gen) < pop_size:
				mutation_section_shuffle(second, 0.2)
	...
	# print(curr_gen[0][1])
	assert is_good_perm(curr_gen[0][1])
	return best, curr_gen[0][1], best_lst

def stand_fit(fitness):
	val = sum(fitness)
	a = len(fitness)
	for i in range(a):
		fitness[i]/=val
def pick_one(p):
	mychoice = random.uniform(0,1)
	i = 0
	val = p[i]
	while mychoice >= val:
		i+=1
...
if __name__ == '__main__':
	...
	best_score, best_perm, best_lst = crossover_search('cities1000.txt', max_iter=7000, pop_size=100)
	elapsed_time = time.time() - start_time
	total = best_score
	#write results 
	print(best_score)
	myfile = open('finalOX_results.txt', 'a')
	myfile.write('sect_shuff\n')
	myfile.write(f'score:{best_score}\n')
	myfile.write(f"time = {elapsed_time}\n")
	myfile.write(f"perm: {best_perm}\n\n")
	myfile.close()

	# #create a graph for this!
	# myfile = open('finalOX_graphing.txt', 'a')
	# myfile.write('sect_shuff\n')	
